...
| D4 | Code opslaan als wachtwoord bij eerste inlog, zodat je hem makkelijk terugvult na uitloggen | **DEELS (v82 tot v84).** Toesteltest 14 sep: een bewaar-knop krijgt iOS niet zover, dus die is in v84 weer weg. Wat blijft: het herstelveld is een wachtwoordveld met een gebruikersnaam per wedstrijd, dus de eerste echte inlog met de code laat de telefoon 'bewaren?' vragen. Middel; het inlogformulier met `autocomplete="username"` en `autocomplete="current-password"` markeren zodat iOS/Android aanbieden hem in de wachtwoordmanager te zetten. Geen account nodig. Hangt samen met D1 |
