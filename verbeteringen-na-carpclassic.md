...
| D4 | Code opslaan als wachtwoord bij eerste inlog, zodat je hem makkelijk terugvult na uitloggen | **AFGEVOERD (14 sep).** Twee varianten getest op iPhone en iPad: iOS vraagt nooit om te bewaren, ook niet bij een echte inlog. Zonder accounts niet te maken. Wat er wél is: sessie-herstel (D1), 'kopieer' en 'stuur naar jezelf' (D12). Was: het inlogformulier met `autocomplete="username"` en `autocomplete="current-password"` markeren zodat iOS/Android aanbieden hem in de wachtwoordmanager te zetten. Geen account nodig. Hangt samen met D1 |
