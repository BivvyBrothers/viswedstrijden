...
def bouw_tenant(slug, kort, volledig, water, stekken, zones, kaart_van):
    ...
    print(f'  2b. PAKKETLIMIET zetten (hoort bij de prijsafspraak; personen, niet teams):')
    print(f"     update wedstrijd.klant_instellingen set max_deelnemers = <10|25|50>")
    print(f"       where klant_id = (select id from wedstrijd.klanten where slug = '{slug}');")
