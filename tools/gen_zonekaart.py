...
# === zonelaag: middellijn + grenslijnen (binnen het water) ===
spine_pts = ' '.join(f"{fmt(T(q)[0])},{fmt(T(q)[1])}" for q in SPINE)
p.append(f'<polyline points="{spine_pts}" fill="none" stroke="#d63a2f" stroke-width="7" stroke-opacity="0.75" stroke-linecap="round" stroke-linejoin="round"/>')
for a, b, m, s_ in grenzen:
    x1, y1 = T(m); x2, y2 = T(s_)
    p.append(f'<line x1="{fmt(x1)}" y1="{fmt(y1)}" x2="{fmt(x2)}" y2="{fmt(y2)}" stroke="#d63a2f" stroke-width="3.2" stroke-opacity="0.75" stroke-linecap="round"/>')
p.append('</g>')
...
# titel + legenda
p.append('<text x="30" y="618" font-size="24" font-weight="700" fill="#123c5e">CONCEPT · Zoneverdeling wedstrijdstekken</text>')
p.append('<text x="30" y="643" font-size="13" fill="#5b5442">22 vaste wedstrijdstekken (groen) · rode lijnen = zonegrenzen, van de oever naar de middellijn</text>')
p.append('<text x="30" y="663" font-size="12" fill="#8a8268">Grenzen automatisch bepaald: precies midden tussen twee buurstekken. Ter vergelijking met de echte lijnen.</text>')
...
p.append('<line x1="840" y1="638" x2="880" y2="638" stroke="#d63a2f" stroke-width="3.2" stroke-opacity="0.75"/><text x="888" y="642" font-size="12" fill="#4a4536">zonegrens</text>')
p.append('</svg>')

with open('zonekaart-concept.svg', 'w') as f:
    f.write(''.join(p))
